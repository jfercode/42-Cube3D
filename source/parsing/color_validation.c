...
/*   Updated: 2025/05/15 12:19:12 by jaferna2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_assign_color(char *cleaned, char flag, t_cub3d *cub3d)
{
	...
	while (color_mtx[i])
	{
		...
		if (temp < 0 || temp > 255)
			return (ft_printf(STDERR_FILENO, "Error:\nInvalid value %d in %c argument\n", temp, flag), ft_free_matrix(color_mtx), FAIL);
		else
		{
			if (flag == 'C')
	...
	if (flag == 'C')
		return (cub3d->ceiling_assigned = true, SUCCESS);
	else if (flag == 'F')
		return (cub3d->floor_assigned = true, SUCCESS);
}
